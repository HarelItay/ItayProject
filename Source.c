...
void fuelConsumption(path_t *path) {
	...
	while (temp != NULL)//while the number of packages is different than zero
	{
		...
		printf("driving to %d", temp->store);
		...
		{//In order to find the appropriate formula, use the Oiler - Lagrange theorem and add the required regular scalars.
			...
			printf("%.3f  %.1f \t %d", fc[i][0], fc[i][1], fc[i][2]);
		}
...
int getFuel() {/*get input from user to set truck fuel and cleare stdin buffer
...
void CheckDig()//Checks if the barcode is valid.
{
...
void * FirstBarcodeDig(char Pixels[])//This function decrypting the first half of the barcode (untill the middle long lines).
{
...
void * SecBarcodeDig(char Pixels[])//This function decrypting the 2nd half of the barcode. from the middle long lines to the end long lines.
{
	int LineJump = BMPinfo.width * 12, i;//Setting up int. LineJump = 4 lines above the first colored pixel.
...
int getOption()//This function checks if the menu input is valid.
{
	char choice;
...
void option1ReadUPC()//This function is for the first menu option. 
{
	FILE * LETSPLAY;
	int i;
